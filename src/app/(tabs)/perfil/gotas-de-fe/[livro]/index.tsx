      ...
      ListHeaderComponent={
        <View className="mb-1 flex-row items-center gap-3 rounded-2xl bg-navy p-4">
          <View className="h-12 w-12 items-center justify-center rounded-full bg-white/10">
            <Ionicons name={book.icon as keyof typeof Ionicons.glyphMap} size={22} color="#7dd3fc" />
          </View>
          <View className="flex-1">
            <Text className="text-sm font-semibold text-white">{book.title}</Text>
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                ...
                params: { livro, numero: String(item.number) },
              })
            }
